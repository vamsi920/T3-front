...
const fetchAndParseData = async (variant) => {
  const query = 'https://api.fda.gov/drug/ndc.json?search=product_ndc:' + variant + '&limit=1';
        ...
        <TableBody>
          {rowsFinal.map((row) => (
            ...
            >
              ...
              <Stack><Button variant="contained" color="success"
              onClick={() => {
              ...
              <TableCell align="left">
                <Stack><Button variant="contained" color="error"
                onClick={() => {
                  const index = rowsFinal.indexOf(row);
                  if (index > -1) {
